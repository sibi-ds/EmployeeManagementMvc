			...
			<table border="1">
				...
				<tbody>
					<tr>
						...
						<td><a
							href="employee?action=get_employee_details&employee_id=${employee.getId()}">
								<button>Update Employee</button>
						</a></td>
						<td><a
							href="assign_project.jsp?employee_id=${employee.getId()}">
								<button>Assign Project</button>
						</a></td>
						<td><a
							href="employee?action=delete_employee&employee_id=${employee.getId()}">
					...
					<tbody>
						<c:forEach items="${employee.getProjects()}" var="project">
							<tr>
								...
								<td><a
									href="employee?action=unassign_project&employee_id=${employee.getId()}&project_id=${project.getId()}">Unassign</a>
								</td>
							<tr>
						</c:forEach>
					</tbody>
